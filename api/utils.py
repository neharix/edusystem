...
def create_hardcore_example(row_count: int, high_school: HighSchool) -> Workbook:
    ...
    department_specializations = DepartmentSpecialization.objects.filter(
        ...
        "faculty_department__high_school_faculty__faculty__id",
        "faculty_department__department__id",
    )
    index = 1
    specialization_ws: Worksheet = wb.create_sheet("Hünarler")
    for specialization in Specialization.objects.filter(
        id__in=[
            department_specialization.id
    ...
    department_ws: Worksheet = wb.create_sheet("Fakultetler")
