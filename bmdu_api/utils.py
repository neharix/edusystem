...
def create_hardcore_example(row_count: int, high_school: HighSchool) -> Workbook:
    wb = Workbook()
    wb.remove(wb.worksheets[0])
    ...
    # Faculties and departments list
    ...
    for row_index in range(2, row_count + 2):
        ...
        study_year_data_val.add(ws[f"K{row_index}"])
...
def validate_hardcore_excel_fields(
    row, index: int, high_school: HighSchool, row_validation: bool
):
    data = {}
    invalid_fields = []
    data["full_name"] = row["F.A.Aa"].rstrip().lstrip()
    if type(row["Doglan senesi"]) == datetime.datetime:
        data["birth_date"] = row["Doglan senesi"]
    elif type(row["Doglan senesi"]) == str:
        try:
    ...
    # Faculty
    try:
        faculty_id = int(row["Fakulteti"].split(".")[0].rstrip().lstrip())
        if Faculty.objects.filter(id=faculty_id).exists():
            data["faculty"] = Faculty.objects.get(id=faculty_id)
        else:
            invalid_fields.append(
                f"Setir №{index + 1}: '{row['Fakulteti']}' atly fakultet tapylmady"
            )
            row_validation = False
    except:
        invalid_fields.append(
            f"Setir №{index + 1}: 'Fakulteti' meýdançasynda ýalňyşlyk goýberildi"
        )
        row_validation = False

    # Department
    try:
        department_id = int(row["Kafedrasy"].split(".")[0].rstrip().lstrip())
        if Department.objects.filter(id=department_id).exists():
            data["department"] = Department.objects.get(id=department_id)
        else:
            invalid_fields.append(
                f"Setir №{index + 1}: '{row['Kafedrasy']}' atly kafedra tapylmady"
            )
            row_validation = False
    except:
        invalid_fields.append(
            f"Setir №{index + 1}: 'Kafedrasy' meýdançasynda ýalňyşlyk goýberildi"
        )
        row_validation = False

    # Specialization
    try:
        specialization_id = int(row["Hünäri"].split(".")[0].rstrip().lstrip())
        if Specialization.objects.filter(id=specialization_id).exists():
            data["specialization"] = Specialization.objects.get(id=specialization_id)
        else:
            invalid_fields.append(
                f"Setir №{index + 1}: '{row['Hünäri']}' atly hünär tapylmady"
            )
