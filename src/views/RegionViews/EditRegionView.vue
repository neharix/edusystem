<template>
  ...
  <div class="w-full rounded-lg shadow-lg p-4 bg-white dark:bg-[#171131ef]">
    <h3 class="text-xl font-bold mx-2 select-none">Welaýaty üýtgetmek</h3>
    <Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }" class="space-y-4 my-4">
      <div class="w-full">
        <Field name="name" type="text" id="name" v-model="regionName"
          ...
          :class="{ 'is-invalid': errors.name }" placeholder="Welaýatyň ady"></Field>
...
const schema = Yup.object().shape({
  name: Yup.string().trim().required('Welaýatyň ady hökman girizilmeli'),
});

function onSubmit(values, { setErrors }) {
  const { name } = values;
  return regionsStore.put(route.params.id, { name }).then(() => {
    router.go(-1);
  }).catch(error => setErrors({ apiError: error }));
}

onMounted(() => {
  regionsStore.get(route.params.id).then(() => {
    regionName.value = regionsStore.region.name;
  })
})
...
const breadcrumbPaths = [
  ...
  { path: "/regions/edit", name: "Üýtgetmek", current: true },
]
