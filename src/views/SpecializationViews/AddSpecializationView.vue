<template>
  ...
  <div class="w-full rounded-lg shadow-lg p-4 bg-white dark:bg-[#171131ef]">
    <h3 class="text-xl font-bold mx-2 select-none">Täze kafedrany hasaba almak</h3>
...
const schema = Yup.object().shape({
  ...
  abbreviation: Yup.string().trim().required('Kadefranyň gysgaltmasy hökman girizilmeli'),
});

function onSubmit(values, {setErrors}) {
  const {name, abbreviation} = values;
  return departmentsStore.create({name, abbreviation}).then(() => {
