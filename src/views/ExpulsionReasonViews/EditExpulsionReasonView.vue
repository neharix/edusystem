<template>
  ...
  <div class="w-full rounded-lg shadow-lg p-4 bg-white dark:bg-[#171131ef]">
    <h3 class="text-xl font-bold mx-2 select-none">Sebäbi üýtgetmek</h3>
    <Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }" class="space-y-4 my-4">
      <div class="w-full">
        <Field name="name" type="text" id="name" v-model="reason"
          ...
          :class="{ 'is-invalid': errors.name }" placeholder="Sebäp"></Field>
...
const schema = Yup.object().shape({
  name: Yup.string().trim().required('Sebäp hökman girizilmeli'),
});

function onSubmit(values, { setErrors }) {
  const { name } = values;
  return expulsionReasonsStore.put(route.params.id, { name }).then(() => {
    router.go(-1);
  }).catch(error => setErrors({ apiError: error }));
}

onMounted(() => {
  expulsionReasonsStore.get(route.params.id).then(() => {
    reason.value = expulsionReasonsStore.expulsionReason.name;
  })
})
...
const breadcrumbPaths = [
  ...
  { path: "/expulsion-reasons/edit", name: "Üýtgetmek", current: true },
]
