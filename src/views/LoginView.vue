...
<template>
  ...
  <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-4 text-center lg:text-left select-none">Hoş geldiňiz! 👋</h2>
  <p class="text-gray-600 dark:text-gray-300 mb-6 text-center lg:text-left select-none">Içeri girmek üçin şahsyňyzy tassyklaň</p>
  <Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }" class="space-y-4">
    <div>
      <label for="username" class="block text-gray-700 dark:text-gray-300 text-sm py-2 select-none">Ulanyjy adyňyz</label>
      <Field name="username"
             type="text"
             id="username"
    ...
    <div class="flex flex-wrap justify-center">
      <button :disabled="isSubmitting"
          ...
          <the-spinner :class="{ hidden: !isSubmitting }"></the-spinner><span class="select-none" :class="{ hidden: isSubmitting }">Giriş</span>
      </button>
    </div>
