<script setup>
import { useGraduatesStore } from '@/stores/api.store';
...
const { studentInfo } = storeToRefs(graduatesStore);
...
onBeforeMount(() => {
  ...
  graduatesStore.getInfo(route.params.id).then(() => {
    if (studentInfo.value.active) {
      ...
      student.studyPeriod = studentInfo.value.study_period;
...
const breadcrumbPaths = [
  ...
  { path: "/graduates/view", name: "Görmek", current: true },
]
...
<template>
  ...
  <div class="tile">
    <div class="flex justify-between">
      <h3 class="text-base md:text-xl font-bold mx-2 select-none">Uçurym barada maglumat</h3>
      <the-spinner v-if="uxStore.isLoading"></the-spinner>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
      <div>
        <p class="info-label">Uçurymyň ady, familiýasy, atasynyň ady</p>
      ...
      <div>
        <p class="info-label">Okan döwri</p>
        <p class="info-content">{{ student.studyPeriod }} ýyllar aralygy</p>
      </div>
