...
watch(deleteStatus, (newVal, oldVal) => {
  if (newVal) {
    if (newVal === 'success') {
      uxStore.addToast('Sebäp üstünlikli ýok edildi', 'success');
...
onMounted(() => {
  if (updateStatus.value) {
    if (updateStatus.value === 'success') {
      uxStore.addToast('Sebäp üstünlikli üýtgedildi', 'success');
  ...
  if (createStatus.value) {
    if (createStatus.value === 'success') {
      uxStore.addToast('Sebäp üstünlikli hasaba alyndy', 'success');
      ...
      <table class="w-full min-w-full table-auto bg-white dark:bg-[#171131ef]">
        ...
        <tbody>
          <tr v-for="(item, index) in paginatedData" :key="index"
            class="transition ease-in hover:ease-out duration-200 hover:bg-white dark:hover:bg-[#261953]"
            :class="{ 'bg-white dark:bg-[#261953]': !item.is_viewed, 'bg-gray-200 dark:bg-[#171131ef]': item.is_viewed }">
